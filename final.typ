...
#let conf(
  ...
  let header = [
    #set text(size: 13pt)
    #stack(dir: ltr, spacing: 15pt, [], align(bottom + left, box(width: 1.35cm, image(logos.escudo))), align(
      bottom + left,
      stack(
        dir: ttb,
        spacing: 5pt,
  ...
  let portada = align(center)[
    #stack(dir: ttb, spacing: 1fr, ..(
      ...
      metadata.espaciado_titulo,
      if metadata.profesores.len() == 0 [#none] else if metadata.profesores.len()
        == 1 [#_guia(gen: metadata.profesores.at(0).pronombre): \ #metadata.profesores.at(0).nombre] else [#_guia(gen: metadata.profesores.at(0).pronombre): \ #metadata.profesores.at(0).nombre \
        #_guia(gen: metadata.profesores.at(1).pronombre) 2: \ #metadata.profesores.at(1).nombre],
      if metadata.coguias.len() == 0 [#none] else if metadata.coguias.len()
        == 1 [#_coguia(gen: metadata.coguias.at(0).pronombre): \ #metadata.coguias.at(0).nombre] else [#_coguia(gen: metadata.coguias.at(0).pronombre): \ #metadata.coguias.at(0).nombre \
        #_coguia(gen: metadata.coguias.at(1).pronombre) 2: \ #metadata.coguias.at(1).nombre],
      if metadata.final.comision.len() == 0 [#none] else if metadata.final.comision.len()
        == 1 [#_comision \ #metadata.final.comision.at(0)] else if metadata.final.comision.len()
        == 2 [#_comision \ #metadata.final.comision.at(0) \ #metadata.final.comision.at(
          1,
        )] else [#_comision \ #metadata.final.comision.at(0) \ #metadata.final.comision.at(
          1,
        ) \ #metadata.final.comision.at(2)],
...
#let resumen(
  ...
  let _documento = if metadata.final.tesis and metadata.final.memoria [#_doble] else if metadata
    .final
    .tesis [#_tesis] else [#_memoria]
  let _resumen = [RESUMEN DE LA #_documento]
  let _anno = if metadata.anno != none [#metadata.anno] else [#datetime.today().year()]
  // añadir bloque de resumen
  stack(dir: ltr, 1fr, block(
    width: 60%,
...
#let dedicatoria(doc) = {
  ...
  stack(dir: ttb, 1fr, align(right, doc), 2fr)
  pagebreak(weak: true)
}
