...
#capitulo(title: "Marco tecnológico y arquitectura de la plataforma")[
  ...
  La decisión de mantener esta separación es importante para la modernización. En lugar de concentrar toda la lógica quirúrgica en una aplicación aislada, la nueva solución se integra con los servicios existentes y utiliza un componente de procesos para coordinar acciones que cruzan límites de dominio. Esto permite conservar la modularidad de la plataforma y, al mismo tiempo, disponer de un punto explícito para ejecutar flujos de negocio.
  ...
  En la arquitectura de Lahuén, los workflows de Temporal se implementan en la capa `middle` utilizando PHP y el SDK de Temporal para ese lenguaje. El proyecto `bpm_worker` registra workflows y actividades en Temporal mediante workers, y utiliza una cola de trabajo para recibir tareas. De este modo, la ejecución del proceso queda fuera del frontend y de los microservicios de dominio, pero puede invocar a estos servicios cuando una actividad lo requiere.
  ...
  == Microservicio BPM y mensajería
  ...
  La plataforma también utiliza mecanismos de mensajería para comunicar eventos entre componentes. En los proyectos relevantes aparecen eventos de dominio asociados, por ejemplo, a cambios en citas o atenciones, y una infraestructura que incluye Kafka y RabbitMQ. Para este informe, basta distinguir sus roles generales: Kafka se utiliza como mecanismo de eventos de negocio entre componentes de la plataforma, mientras que RabbitMQ forma parte de la infraestructura de colas disponible en el entorno. Temporal, por su parte, introduce su propio modelo de ejecución mediante workflows, actividades, workers y colas de tareas.

  Esta combinación permite separar dos tipos de comunicación. Los eventos comunican que algo ocurrió en el sistema y permiten que otros componentes reaccionen. Los workflows, en cambio, coordinan acciones que deben ejecutarse como parte de un proceso. En el módulo quirúrgico ambas ideas son útiles: algunos cambios pueden publicarse como eventos de negocio, mientras que ciertas acciones críticas requieren una coordinación explícita y durable.

  == Orquestaciones dinámicas y DSL

  Dentro de Temporal, el trabajo incorpora un workflow particular llamado orquestador dinámico. Este workflow está implementado en PHP y su función es interpretar orquestaciones definidas como configuraciones. En vez de crear un workflow distinto para cada acción de negocio, el orquestador dinámico recibe una definición y la ejecuta como una secuencia de instrucciones simples, tales como llamadas HTTP, asignaciones de valores, condiciones de ejecución y mapeos de datos entre respuestas previas y pasos posteriores.
  ...
  Esta organización permite distinguir responsabilidades. La interfaz presenta pacientes, estados y acciones disponibles. Los microservicios de dominio ejecutan operaciones sobre datos clínicos y administrativos. BPM inicia workflows. Temporal asegura la ejecución durable. Los workers procesan workflows y actividades. El workflow de orquestador dinámico interpreta las configuraciones que describen la secuencia concreta de instrucciones para cada acción quirúrgica configurable.

  En consecuencia, la arquitectura tecnológica escogida no es un conjunto aislado de herramientas, sino una respuesta a las limitaciones identificadas en la situación inicial. El uso de microservicios permite integrarse con la plataforma existente; Vue 2 y la arquitectura de plugins permiten construir una interfaz modular; BPM entrega un punto común para instanciar workflows; Temporal permite ejecutar procesos distribuidos de forma durable; y el workflow de orquestador dinámico permite expresar las acciones quirúrgicas como configuraciones interpretables. Estos elementos serán utilizados en los capítulos siguientes para definir los requerimientos, el diseño y la implementación de la nueva solución.
]
